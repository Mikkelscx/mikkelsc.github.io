...
document.addEventListener('DOMContentLoaded', function() {
	...
	function createConnectingLines() {
		...
		projectNodes.forEach((node, index) => {
			...
			// Direction from node center towards brain center
			const dx = brainCenterX - nodeCenterX;
			const dy = brainCenterY - nodeCenterY;
			...
			// Endpoint on the edge of the node (tap point)
			const tapX = nodeCenterX + dx * t;
			const tapY = nodeCenterY + dy * t;
			
			// Calculate line coordinates relative to SVG
			const svgRect = connectingLines.getBoundingClientRect();
			const startX = brainCenterX - svgRect.left;
			const startY = brainCenterY - svgRect.top;
			const endX = tapX - svgRect.left;
			const endY = tapY - svgRect.top;
	...
	setTimeout(createConnectingLines, 150); // Small delay to ensure elements are positioned
